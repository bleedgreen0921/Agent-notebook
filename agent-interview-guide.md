...
### 展开解释：用户一句话进来后到底发生了什么

这张图的重点不是组件名字，而是责任边界。面试官问“用户一句话进来后发生了什么”，本质是在确认你是否知道 **Agent 不是一次模型调用，而是一条受控的后端执行链路**。一个成熟回答要讲清楚三件事：**请求怎么进来，模型在哪里做决策，真实动作由谁执行和兜底**。

#### 用户请求层：先把自然语言变成受控任务

用户输入通常不是干净的函数参数，而是一段自然语言，例如“帮我查一下上周上海客户的异常订单，能退款的先生成退款申请”。这一句话里可能混着时间范围、地域、业务对象、动作意图和隐含风险。系统不能直接把它丢给模型执行，而要先进入后端入口层。

入口层通常会做：

- 解析会话：识别这是新任务、继续旧任务，还是对上一步结果的追问。
- 识别用户和租户：确认 `user_id`、`tenant_id`、角色、组织范围。
- 建立任务对象：生成 `task_id`、`trace_id`，记录 prompt 版本、模型版本和入口来源。
- 设置预算：限制最大模型调用次数、最大工具调用次数、总超时、token 和成本。

面试表达可以这样说：

> 我不会让自然语言请求直接驱动工具执行。入口层会先把请求包成一个有身份、有预算、有 trace 的任务对象，后面的模型调用和工具调用都挂在这个任务下面。

#### 网关层：把安全和流量问题挡在 Agent 外面

`API Gateway / Auth / Rate Limit` 是 Agent 系统的第一道工程边界。它不理解复杂任务，但负责回答几个基础问题：

- 这个请求是谁发的？
- 用户是否登录，token 是否有效？
- 这个租户是否有权限使用 Agent 功能？
- 当前用户或租户是否超过 QPS、并发数、成本额度？
- 请求体大小、文件大小、附件类型是否合法？

**这层不要依赖模型判断权限。模型可以理解“用户想做什么”，但不能决定“用户有没有权做”。权限必须由确定性的后端代码、策略系统或 IAM/RBAC/ABAC 判断。**

#### 任务入口层：建立状态、预算和审计上下文

通过网关后，请求会进入任务入口。这里会把一次对话请求升级成一个可追踪的 Agent 任务。它通常会初始化：

| 字段 | 作用 |
| --- | --- |
| `trace_id` | 贯穿日志、模型调用、工具调用和最终回答 |
| `task_id` | 标识一次可恢复的业务任务 |
| `session_id` | 关联多轮会话上下文 |
| `tenant_id` / `user_id` | 做权限、隔离和审计 |
| `budget` | 限制步数、耗时、成本、工具次数 |
| `state_version` | 支持 checkpoint 和状态迁移 |

这一层的关键是：不要把 Agent 看成“函数调用”，而要看成“任务执行”。任务有状态、有生命周期、有失败原因，也可能进入等待人工确认的中间态。

#### 意图识别、风险分类与路由：决定走哪条执行路径

不是所有请求都应该进入完整 Agent Loop。有些请求可以直接回答，有些只需要 RAG，有些需要工具，有些必须人工确认，有些应该拒绝。

常见路由包括：

- 普通问答：直接模型回答，不加载高风险工具。
- 知识库问答：进入 RAG 流程，要求证据和引用。
- 业务查询：加载只读工具，如订单、工单、CRM 查询。
- 写操作：进入草稿或审批流程，如发邮件、退款、修改状态。
- 高风险请求：触发人工确认或安全拒绝。
- 信息不足：先向用户澄清，而不是猜参数。

风险分类的价值是降低工具误调用和安全事故。例如“帮我删除这些记录”和“如何删除记录”语义接近，但前者是写操作请求，后者可能只是咨询。系统应根据动作对象、权限、风险等级和上下文决定是否继续。

#### Agent Orchestrator：模型决策和工程控制的交界处

`Agent Orchestrator` 是整条链路的核心，但它不应该是一个黑盒。它负责把模型、工具、记忆、检索、状态和安全策略组织成一个可控循环。

可以把它拆成几个职责：

| 组件 | 主要职责 | 面试要点 |
| --- | --- | --- |
| Planner | 判断是否需要拆任务、生成步骤或下一步目标 | 简单任务不一定需要显式规划 |
| Tool Selector | 根据工具 schema、用户上下文和策略选择候选工具 | 不要一次暴露所有工具 |
| Executor | 真正调用工具，处理超时、重试、幂等和错误 | 模型不执行函数，代码执行 |
| Memory Manager | 管理短期上下文、会话摘要、长期用户记忆 | 记忆不是权威事实来源 |
| Retriever | 做 BM25、向量、混合检索、rerank 和引用账本 | RAG 要看证据，不只看答案 |
| Guardrails | 做权限、敏感信息、人工确认和输出校验 | 安全不能只靠 prompt |
| State Store | 保存步骤状态、工具结果、checkpoint 和错误原因 | 支持恢复、审计和排障 |

这里最容易被追问的是“模型到底做什么”。稳妥回答是：

> 模型负责在受限上下文里生成候选决策，比如是否需要检索、调用哪个工具、参数是什么、是否已经可以回答。Orchestrator 负责校验这个决策，真正执行工具，并把结果作为 observation 写回状态。

#### Planner：不是每个任务都需要复杂规划

Planner 的作用是判断任务是否要拆解，以及拆成哪些可执行步骤。例如“总结这篇文档”可能不需要显式规划；“分析异常订单、找原因、生成处理建议、创建工单草稿”就适合拆成多个步骤。

面试时要避免把规划说成万能能力。Planner 可能生成错误计划，也可能让简单任务变慢。更合理的说法是：

- 简单请求直接走模型或单工具。
- 多步骤任务可以先生成粗计划。
- 每一步执行后根据工具结果动态修正。
- 计划只是候选，不是不可变流程。
- 计划完成、预算耗尽、需要人工输入时都要停止。

#### Tool Selector 与 Executor：选择工具和执行工具必须分开

Tool Selector 解决“该不该用工具、用哪个工具”的问题；Executor 解决“能不能执行、怎么执行、失败怎么办”的问题。两者不能混在模型里。
...
所以面试中可以强调：模型只提出工具调用意图，工具执行器才是可信执行边界。

#### Memory 与 Retriever：上下文、记忆和知识库要分清

很多候选人会把“聊天历史”“用户记忆”“知识库文档”混成一类上下文。生产系统里要分开：

- 短期上下文：当前任务中的消息、工具结果、临时约束。
- 会话记忆：本轮对话之前的摘要，帮助理解用户连续追问。
- 长期记忆：用户明确保存的偏好和长期信息。
- 业务事实：订单、合同、工单、库存等权威系统数据。
- 知识库证据：文档检索结果，需要引用和权限过滤。

面试时可以说：

> 我会让 Memory Manager 负责“用户和会话上下文”，让 Retriever 负责“外部知识证据”。长期记忆不能直接当业务事实，涉及订单、工单、政策等事实时仍要查权威系统或知识库，并保留引用。

#### Guardrails：把模型放在权限和策略里面

Guardrails 不只是内容安全，也包括业务安全。常见拦截点有：

- 工具调用前：校验权限、风险等级、参数和租户范围。
- 工具调用后：过滤敏感字段，标准化错误，不把内部堆栈暴露给模型或用户。
- 最终回答前：检查引用、敏感信息、格式、是否声称完成了未执行动作。
- 写操作前：展示动作预览，让用户确认。

一个高质量回答要体现：不是模型“承诺安全”，而是系统“强制安全”。例如模型说“我已经退款了”不可信，只有工具执行日志里存在成功的退款记录，系统才允许最终回答里说退款申请已提交。

#### State Store：让 Agent 可恢复、可审计、可排障

Agent 线上最怕的是执行到一半失败：模型调用成功但工具超时，工具写入成功但回传失败，用户审批时页面关闭，worker 重启后重复执行写操作。State Store 的意义就是让这些中间状态可见、可恢复。

它至少要记录：

- 当前执行到哪个节点或步骤。
- 已经调用过哪些工具。
- 每个工具的输入摘要、输出摘要、状态和 `call_id`。
- 写操作的幂等键。
- 检索到的证据 ID。
- 等待人工确认的动作和参数。
- 失败类型、重试次数和最后错误。
...
> Checkpoint 不是为了保存一大坨聊天记录，而是为了恢复执行、避免重复写、支持审计和定位失败。

#### 最终回答层：回答前要做一致性检查

最终回答不是模型生成完就直接返回。系统通常会做几类后处理：

- RAG 场景检查引用 ID 是否存在。
- 工具场景检查最终回答是否和工具结果一致。
- 写操作场景检查是否真的执行成功，还是仅生成草稿。
- 安全场景过滤敏感信息。
- 结构化输出场景校验 JSON Schema。
- 用户体验层面把技术错误转成可理解的失败说明。

例如工具返回的是 `HUMAN_APPROVAL_REQUIRED`，最终回答就不能写“已完成退款”，而应写“已生成退款申请草稿，等待你确认后提交”。

#### 日志、Trace 和评测回流：上线后靠数据改进

最后一层“日志、Trace、指标、评测样本回流”决定系统能不能持续变好。Agent 的失败原因可能在模型、检索、工具、权限、状态或外部系统任意一层。没有 trace，就只能猜。
...
> 用户请求进来后，我不会直接把自然语言交给模型执行。网关先做鉴权、租户识别和限流，入口层创建带 `trace_id`、预算和状态的任务。然后做意图识别和风险分类，决定是普通问答、RAG、只读工具、写操作还是人工确认。进入 Agent Orchestrator 后，模型只负责生成候选决策，比如是否检索、调用哪个工具、参数是什么；后端会校验工具名、参数、权限、风险等级和租户边界，再由 Executor 真正调用外部 API 或数据库。工具结果会带 `call_id` 写回 State Store，必要时 checkpoint，模型再基于 observation 继续下一步。最终回答前会做引用校验、敏感信息过滤、动作状态对账和日志落盘。这样每一步都能追踪、恢复、审计，也能用指标持续优化。
